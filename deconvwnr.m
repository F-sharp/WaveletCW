...
%   Class Support
%   -------------
%   I can be uint8, uint16, int16, double, or single. Other inputs have
%   to be double. J has the same class as I.
%
%   Example
%   -------
%
...
% Third, reconstruct the image.  The second argument in the calculation for
% denom is necessary in the event that K is an array instead of a scalar. When K
% is an array, it is the same size as the image (see g280432).
...
for n = 2:nargin,
    iptcheckinput(varargin{n},{'double'},{'real' 'finite'},...
...
sizeI = size(I);
classI = class(I);
iptcheckinput(I,{'uint8','uint16','int16','double','single'},{'real' ...
                    'finite'},mfilename,'I',1);
if prod(sizeI)<2,
  eid = sprintf('Images:%s:mustHaveAtLeast2Elements',mfilename);
  error(eid,'In function %s, input image must have at least two elements.', ...
                mfilename);
elseif ~isa(I,'double')
  I = im2double(I);
end
...
if prod(sizePSF)<2,
  eid = sprintf('Images:%s:mustHaveAtLeast2Elements',mfilename);
  error(eid,'In function %s, PSF must have at least two elements.',mfilename);
elseif all(PSF(:)==0),
  eid = sprintf('Images:%s:psfMustNotBeZeroEverywhere',mfilename);
  error(eid,'In function %s, PSF cannot be zero everywhere.',mfilename);
end
...
if isempty(ncorr) && ~isempty(icorr),
  ...
  error(eid, ...
        'Invalid input in function %s: provide characteristics for noise.', ...
        mfilename);
end
...
if any(sizeI(numNSdim) < sizePSF(numNSdim))
  ...
  error(eid,'In function %s, size of the PSF must not exceed the image size in any nonsingleton dimension.', mfilename);
end

if isempty(icorr) && (prod(sizeNCORR)>1) && ~isequal(sizeNCORR,sizeI)
    eid = sprintf('Images:%s:nsrMustBeScalarOrArrayOfSizeA',mfilename);
    error(eid, ...
          'DECONVWNR(A,PSF,NSR): NSR has to be a scalar or an array of size A.');
end
